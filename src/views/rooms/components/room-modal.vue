<template>
    <el-dialog class="c-team-info-modal" :title="title" :visible.sync="visibleModal" :before-close="handleClose" width="450px"
        ...
        <div v-loading="loadingModal">
            <el-form :model="team" label-width="70px" size="mini">
                ...
                <el-form-item label="最大人数" prop="desc">
                    <el-slider v-model="team.number" :min="1" :max="6" show-input>
                    </el-slider>
                </el-form-item>
            </el-form>
            <div class="actions">
                <el-button icon="el-icon-close" @click="onClose">取 消</el-button>
                <el-button icon="el-icon-check" type="primary" :loading="saveLoading" @click="onSaveTeam">创 建</el-button>
            </div>
        </div>
    </el-dialog>
</template>
<script>
    import BaseModal from '@/mixins/common/base-model';
    ...
    export default {
        ...
        computed: {
            title() {
                return this.team.id > 0 ? '修改房间' : '创建房间';
            }
        },
        data() {
            return {
